...
class CustomDate
{
    ...
    /**
     * Calcola la differenza tra 2 date (data di inizio e data di fine), ritorna la differenza in anni (oppure mesi, giorni ...)
     *
    ...
    static function add($datetime, $days, $format_out = "Y-m-d")
    {
        ...
        date_add($data, date_interval_create_from_date_string("$days days"));
     ...
     * @return string
     */
    static function sub($datetime, $days, $format_out = "Y-m-d", $format_in = "Y-m-d H:i:s")
    {
        $data = date_create_from_format($format_in, $datetime);
     ...
     * @return boolean
     */
    static function isSuccessiva($datetime1, $format1, $datetime2, $format2)
    {
        if (empty($datetime1) || empty($datetime2) || $datetime1 == "0000-00-00" || $datetime2 == "0000-00-00")
            return 0;
        $data1 = date_create_from_format($format1, $datetime1);
    ...
    /**
     * Ritorna lo stesso giorno per l'anno successivo, es.
     * se 30/05/2021 è Domenica, ritorna 29/05/2022 (domenica)
     *
     * @param \DateTime $date
     * @return \DateTime
     */
    static function getSameDayNextYear(\DateTime $date = null)
    {
        if (! $date) {
            $date = new \DateTime();
        }
