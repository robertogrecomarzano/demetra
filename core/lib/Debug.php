...
class Debug
{
    ...
    public static function var_dump($var, $return = true, $expandLevel = 1)
    {
        self::$hasArray = false;
        ...
        $html = '<pre style="margin-bottom: 18px;' . 'background: #f7f7f9;' . 'border: 1px solid #e1e1e8;' . 'padding: 8px;' . 'border-radius: 4px;' . '-moz-border-radius: 4px;' . '-webkit-border radius: 4px;' . 'display: block;' . 'font-size: 12.05px;' . 'white-space: pre-wrap;' . 'word-wrap: break-word;' . 'color: #333;' . 'font-family: Menlo,Monaco,Consolas,\'Courier New\',monospace;">';
        $done = array();
     ...
     * like booleans and resources.
     * Supports collapsable arrays and objects
     * as well.
     *
     * @param mixed $var
    ...
    public static function var_dump_plain($var, $expLevel, $depth = 0, $done = array())
    {
        ...
        if (is_bool($var)) {
            ...
            $html .= '<span style="color:#588bff;">string</span><span style="color:#999;">(</span>' . strlen($var) . '<span style="color:#999;">)</span><br /><strong>' . self::htmlentities($var) . '</strong>';
        } elseif (is_null($var)) {
